function [J grad] = nnCostFunction_selective(nn_params, nn_lsizes, X, y, lambda)
...
% COST
%%% COST FUNCTION FOR "SELECTIVE" BACK PROPAGATION
LastLayerOutput_selective = y.*   a(:,end-nn_lsizes(end)+1:end);
%%%
Jvec_h = LastLayerOutput_selective(:);
Jvec_y = y(:);
...
% GRADIENTS
%%% "SELECTIVE" BACK PROPAGATION: Doesn't look at moves it didn't make, only at the move it did:
LastLayerErrors_selective = (y.*    (a(:,end-nn_lsizes(end)+1:end) - y))';
%%%

grad = (  1/size(a,1)  *  LastLayerErrors_selective  *  [ones(size(a,1),1) a(:,indices_a(1,size(nn_lsizes,2)-1):indices_a(2,size(nn_lsizes,2)-1))]  )(:);
