function gameFlag = acquireLEARNING(player1, player2)
...
sub_gameFlag = -1;
if 1
    LEARNING_DATASIZE = 10000;
    LEARNING_INPUT=[];
    LEARNING_OUTPUT=[];
    while LEARNING_DATASIZE--
        printf('%d\r',LEARNING_DATASIZE); if exist('OCTAVE_VERSION'), fflush(stdout); end
        ...
        if( sub_gameFlag==0 && sub_moveNo==7)    
            ...
            if ((flag1==2 || flag2==2) && !((flag1==2 && flag2==2)))
                LEARNING_INPUT=[LEARNING_INPUT; sub_gameState(:)'];
                if (flag1==2) LEARNING_OUTPUT=[LEARNING_OUTPUT; gS(:)']; end
                if (flag2==2) LEARNING_OUTPUT=[LEARNING_OUTPUT; gS2(:)']; end
            end
        end
    end
    save LEARNING_INPUT
    save LEARNING_OUTPUT
    printf('\n');
else if 0
